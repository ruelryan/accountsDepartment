...
interface StaffSectionProps {
  ...
  onStatusChange: (personId: string, newStatus: string) => void;
}

export function StaffSection({ title, people, roleType, activeShift, onStatusChange }: StaffSectionProps) {
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      ...
      <div className="space-y-3">
        {filteredPeople.map(person => {
          const currentRole = person.roles.find(role => role.type === roleType);
